<template>
  <nav class="navbar">
    ...
    <router-link class="login_btn" :to="isAuthenticated ? '/' : '/LoginSignUp'"
    ...
    async checkAuth() {
      auth.onAuthStateChanged(async (user) => {
        if (user) {
          ...
          this.userName = user.displayName || user.email; // Fallback to email if displayName is not set
        } else {
          this.userName = null; // User is signed out
        }
      });
    },
  },
